...
import { getCharacterData, saveCharacterData } from '@/utils/localStorage';
...
export default function Home() {
  ...
  const [found, setFound] = useState<Partial<Record<CharacterNameKeys, boolean>>>({} as any);
  
  // State for tracking egg codes
  const [codes, setCodes] = useState<Partial<Record<CharacterNameKeys, string[]>>>({} as any);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {characters.map((character) => (
          <CharacterCard
            ...
            id={character.id}
            isFound={Boolean(found[character.characterCode])}
